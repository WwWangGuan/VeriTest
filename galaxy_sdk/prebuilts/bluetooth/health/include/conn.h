...
#if defined(CONFIG_BT_SMP_PAIRING_PARAM_SET)
/** @brief Allow/disallow mitm to be used for pairing.
 *
 *  Set/clear the MITM flag for SMP Pairing Request/Response data.
 *  A device sets the MITM flag to one to request an Authenticated 
 *  security property for the STK when using LE legacy pairing and 
 *  the LTK when using LE Secure Connections.
 *
 *  @param enable Value allowing/disallowing to be bondable.
 */
void bt_set_mitm_flag(bool enable);

/** @brief Enable/disable key press notification to be used for pairing.
 *
 *  Set/clear the Key Press Notification flag for SMP passkey entry,
 *  If the flag was set, device will notify peer device the state of passkey entry.
 *
 *  @param enable Value allowing/disallowing to be bondable.
 */
void bt_set_key_press_notification_flag(bool enable);
#endif /* CONFIG_BT_SMP_PAIRING_PARAM_SET */
...
#if defined(CONFIG_BT_VS_EXT_CALLBACK)
struct bt_conn_vs_cb {
	/** @brief notify that the number of transmission packet buffers.
	 *
	 *  This callback notifies the application that the number of the
	 *  available transmission packet buffers when send a packet.
	 *
	 *  @param count the number of the available transmission 
	 *               packet buffers.
	 */
	void (*tx_buffer_reduced)(uint32_t count);
};

/** @brief Register VS callbacks.
 *
 *  Register callbacks for VS use. 
 *
 *  @param cb Callback struct.
 *
 *  @return Zero on success or negative error code otherwise
 */
int bt_conn_vs_cb_register(const struct bt_conn_vs_cb *cb);
#endif /* CONFIG_BT_VS_EXT_CALLBACK */

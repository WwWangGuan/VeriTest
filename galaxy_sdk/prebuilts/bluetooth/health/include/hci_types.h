...
#define BT_HCI_OP_LE_ENHANCED_READ_TX_POWER_LEVEL	BT_OP(BT_OGF_LE, 0x0076)
struct bt_hci_cp_le_enhanced_read_tx_power_level {
	uint16_t handle;
	uint8_t  phy;
} __packed;

struct bt_hci_rp_le_enhanced_read_tx_power_level {
	uint8_t  status;
	uint16_t handle;
	uint8_t  phy;
...
#define BT_HCI_EVT_LE_PATH_LOSS_THR     		0x20
struct bt_hci_evt_le_path_loss_thr {
	uint16_t conn_handle;
	uint8_t  current_path_loss;
	uint8_t  zone_entered;
} __packed;

#define BT_HCI_EVT_LE_TRANSMIT_POWER_REPORTING  0x21
struct bt_hci_evt_le_transmit_power_reporting {
	uint8_t  status;
	uint16_t conn_handle;
	uint8_t  reason;
	uint8_t  phy;

...
#ifdef CONFIG_BT_GATT_EXT_CALLBACK
	/* server receive access database req from peer */
	void (*att_peer_access_db)(struct bt_conn *conn, uint16_t handle,
			uint8_t op,	const void *data, uint16_t length);
	/* notify without subscrite */
    void (*att_val_global_notify)(struct bt_conn *conn, uint16_t handle,
			const void *data, uint16_t length);
	/* indicate without subscrite */
	void (*att_val_global_ind)(struct bt_conn *conn, uint16_t handle,
			const void *data, uint16_t length);
	/* client receive ATT_PREPARE_WRITE_RSP PDU */
	void (*att_prepare_write_rsp)(struct bt_conn *conn, uint16_t handle,
			const void *pdu, uint16_t length);
	/* server send ATT_READ_TYPE_RSP PDU */
	void (*att_read_by_type_comp)(struct bt_conn *conn, uint16_t handle);
	/* server send ATT_READ_RSP PDU */
	void (*att_read_comp)(struct bt_conn *conn, uint16_t handle);
	/* server send ATT_READ_BLOB_RSP PDU */
	void (*att_read_blob_comp)(struct bt_conn *conn, uint16_t handle);
	/* server send ATT_READ_MULTI_RSP PDU */
	void (*att_read_multi_comp)(struct bt_conn *conn, uint8_t count, uint16_t *attr_hdl_list);
	/* server send ATT_WRITE_RSP PDU */
	void (*att_write_comp)(struct bt_conn *conn, uint16_t handle);
	/* server send ATT_PREPARE_WRITE_RSP PDU */
	void (*att_prepare_write_comp)(struct bt_conn *conn, uint16_t handle,
			uint16_t offset, uint16_t length);
	/* server send finished Write Long Characteristic value */
	void (*att_exe_write_comp)(struct bt_conn *conn, uint8_t flags);
#endif
	sys_snode_t node;
};
